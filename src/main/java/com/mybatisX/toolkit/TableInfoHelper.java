...
public class TableInfoHelper {
	...
	public synchronized static TableInfo initTableInfo(Class<?> clazz) {
		...
		for (Field field : list) {
			 ...
			 * 主键ID
			 */
			PK tableId = field.getAnnotation(PK.class);
			if (tableId != null) {
				if (tableInfo.getKeyColumn() == null) {
					...
					/* 发现设置多个主键注解抛出异常 */
					throw new MybatisXException("There must be only one, Discover multiple @TableId annotation in " + clazz);
				}
			}
			...
			if (tableField != null) {
				...
				if (null != columns && null != els && columns.length == els.length) {
					...
					throw new MybatisXException(String.format(errorMsg, clazz.getName(), field.getName()));
				}
